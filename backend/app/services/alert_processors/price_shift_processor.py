...
class PriceShiftProcessor(AlertProcessorInterface):
    ...
    def evaluate_and_generate_alerts(
        ...
        shift_query = text("""
            ...
            FROM LatestPrices l
            JOIN PreviousPrices p ON l.commodity_id = p.commodity_id AND l.market_id = p.market_id
        ...
        if user_id:
            target_users_query = text(str(target_users_query) + f" AND u.id = '{user_id}'")

        target_users = db.execute(target_users_query).fetchall()
