...
class AlertService:
    ...
    @classmethod
    def _send_alert_push_notification(cls, db: Session, alert: Alert) -> int:
        from app.models.user_fcm_token import UserFCMToken
        from app.services.firebase_service import send_fcm_notification

        prefs = db.query(NotificationPreference).filter(NotificationPreference.user_id == alert.user_id).first()
        if prefs and not prefs.delivery_push:
            logger.info(f"Skipping push notification for user {alert.user_id}: delivery_push disabled.")
            return 0

        user_tokens = db.query(UserFCMToken.fcm_token).filter(UserFCMToken.user_id == alert.user_id).all()
        tokens = [t[0] for t in user_tokens if t[0]]
        if not tokens:
            logger.info(f"No FCM tokens found for user {alert.user_id}.")
            return 0
        ...
        return send_fcm_notification(tokens=tokens, title=title, body=message, data=payload_data)
